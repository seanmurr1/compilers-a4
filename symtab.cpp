...
void Symbol::set_offset(unsigned offset) {
  m_requires_storage = true;
  m_offset = offset;
}

bool Symbol::requires_storage() {
  return m_requires_storage;
}

int Symbol::get_vreg() {
  //assert(!m_requires_storage);
  return m_vreg;
}

unsigned Symbol::get_offset() {
  //assert(m_requires_storage);
  return m_offset;
}
